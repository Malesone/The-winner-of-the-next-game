...
class TextAnalysis:
    ...
    def get_predictions(self):
        ...
        for key_date, links_of_predictions in tqdm(self.links_of_pages_by_date.items()):
            for prediction_link in links_of_predictions:
                ...
                self.df.at[count, 'team1'] = self.check_name(home)
                self.df.at[count, 'team2'] = self.check_name(away)
        ...
        self.df.to_csv("all_correspondeces.csv")
    ...
    def recovery_games(self):
        ...
        for k, match in self.matches.iterrows():
            h_team, a_team = match.team1, match.team2
            recovery = recoveries[(recoveries['team1']==h_team) & (recoveries['team2']==a_team)]
            if len(recovery) > 1:
                recoveries.drop((recoveries[(recoveries['team1']==h_team) & (recoveries['team2']==a_team)])[:1].index, inplace=True)
    ...
    def matches_not_found(self):
        ...
        for k, res in self.matches.iterrows():
            if len(self.recoveries[(self.recoveries['team1']==res.team1) & (self.recoveries['team2']==res.team2)]) < 1:
                
                print(res.date, res.team1, res.team2) #stampo quali sono i match che non sono stati trovati
                ...
                soup = BeautifulSoup(pageTree.text, features="lxml")
                links = soup.find_all('a', href=True)
                links = [link['href'] for link in links if (res.team1.lower() in link['href']) or (res.team2.lower() in link['href'])]
                cleaned_links = list(dict.fromkeys(links))
                if len(cleaned_links) > 0: 
                    home, away, text, label = self.get_prediction(cleaned_links[0])
                    
                    self.recoveries.at[count, 'date'] = res.date
                    self.recoveries.at[count, 'team1'] = self.check_name(home)
                    self.recoveries.at[count, 'team2'] = self.check_name(away)
